...
class Replacement_petty_cash(models.Model):
    ...
    def unlink(self):
        if self.facturas_ids:
            raise UserError(
                'No puede Eliminar la Reposición ya que tiene Facturas asociadas a ella')
        for invoice in self:
            if invoice.petty_cash_status not in ('close', 'liquidado'):
                raise UserError(
                    'No se puede Eliminar una Reposición que no este En Proceso')
        return super(Replacement_petty_cash, self).unlink()
