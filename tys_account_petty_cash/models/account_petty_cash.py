...
class Account_petty_cash(models.Model):
    ...
    disponible = fields.Monetary('Saldo de Apertura Disponible')
    ...
    def validate_petty_cash_amount_open(self):
        ...
        if self.petty_cash_amount_open <= 0:
            ...
            cuenta_contable = self.env['account.move.line'].search([('account_id','=', self.petty_cash_account_id.id)])
            cuenta_transitoria = self.env['account.move.line'].search([('account_id', '=', self.petty_cash_trans_account_id.id)])
            for a in cuenta_contable:
                suma += a.balance
            for b in cuenta_transitoria:
                suma1 += b.balance
            self.disponible_move_id = suma + self.petty_cash_amount_open
        return True
    ...
    @api.multi
    def unlink(self):
        code = self.env['replacement.petty.cash'].search([('code','=', self.id)])
        if code:
            raise UserError(
                'No puede Eliminar la Caja chica ya que tiene Reposiciones asociadas a ella')
        facturas = self.env['invoice.petty.cash'].search([('code','=', self.id)])
        if facturas:
            raise UserError(
                'No puede Eliminar la Caja chica ya que tiene Facturas asociadas a ella')

        for invoice in self:
            if (invoice.petty_cash_status not in ('draft', 'cancel')):
                raise UserError(
                    'No se puede borrar una Caja Chica que no sea borrador o cancelada. Debe crear un asiento de reversión en su lugar.')

        return super(Account_petty_cash, self).unlink()
    ...
    def cancel_petty_cash(self):
        ...
        if code:
            raise UserError(
                'No puede Cancelar la Caja chica ya que tiene Reposiciones asociadas a ella')
        facturas = self.env['invoice.petty.cash'].search([('code', '=', self.id)])
        if facturas:
            raise UserError(
                'No puede Cancelar la Caja chica ya que tiene Facturas asociadas a ella')
        for invoice in self:
            if (invoice.petty_cash_status not in ('draft', 'cancel')):
                raise UserError(
                    'No se puede borrar una Caja Chica que no sea borrador o cancelada. Debe crear un asiento de reversión en su lugar.')
    ...
    def confirm_petty_cash(self):
        ...
        #petty_cash = self.env['account.petty.cash'].search([('petty_cash_account_id', '=', self.petty_cash_account_id.id),
                  #                                              ('petty_cash_status','in', ('approved','validate'))])
        #if petty_cash:
