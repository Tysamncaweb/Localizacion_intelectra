...
class hr_payslip2(models.Model):
    _inherit ='hr.payslip'
    ...
    def compute_sheet(self):
        self.structure = self.env['hr.payroll.structure'].search([('code', '=', 'ESP-05')])
        res = super(hr_payslip2, self).compute_sheet()
    ...
    def fields_view_get(self, view_id=None, view_type=False, toolbar=False, submenu=False):
        ...
        return super(hr_payslip2, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
    ...
    def create(self,values, context=None):
        ...
        res = super(hr_payslip2, self).create(values)
        return res
    ...
    def write(self, values):
        ...
        if self.env.context.get('is_payoff', False):
            values.update({'is_payoff': True})
        res = super(hr_payslip2, self).write(values)
        return res
